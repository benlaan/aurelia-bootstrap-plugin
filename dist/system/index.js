System.register(['./alert/bs-alert', './badge/bs-badge', './breadcrumb/bs-breadcrumb', './breadcrumb/bs-breadcrumb-item', './button/bs-button', './button/bs-button-group', './drop-down/bs-drop-down', './drop-down/bs-drop-down-item', './input-controls/bs-form-group', './glyph/bs-glyph', './input-controls/bs-textarea', './input-group/bs-input-group', './jumbotron/bs-jumbotron', './label/bs-label', './list-group/bs-list-group', './list-group/bs-list-group-item', './media-object/bs-media-object', './nav-bar/bs-nav-bar', './page-header/bs-page-header', './pagination/bs-pagination', './panel/bs-panel', './pill/bs-pill', './pill/bs-pill-item', './progress-bar/bs-progress-bar', './responsive-embed/bs-responsive-embed', './grid/bs-row', './grid/bs-column', './grid/bs-container', './tab/bs-tab', './tab/bs-tab-item', './thumbnail/bs-thumbnail', './toolbar/bs-toolbar', './well/bs-well'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var BsAlert, BsBadge, BsBreadcrumb, BsBreadcrumbItem, BsButton, BsButtonGroup, BsDropDown, BsDropDownItem, BsFormGroup, BsGlyph, BsTextArea, BsInputGroup, BsJumbotron, BsLabel, BsListGroup, BsListGroupItem, BsMediaObject, BsNavBar, BsPageHeader, BsPagination, BsPanel, BsPill, BsPillItem, BsProgressBar, BsResponsiveEmbed, BsRow, BsColumn, BsContainer, BsTab, BsTabItem, BsThumbnail, BsToolbar, BsWell;
    function configure(config) {
        config.globalResources('./alert/bs-alert', './badge/bs-badge', './breadcrumb/bs-breadcrumb', './breadcrumb/bs-breadcrumb-item', './button/bs-button', './button/bs-button-group', './drop-down/bs-drop-down', './drop-down/bs-drop-down-item', './glyph/bs-glyph', './grid/bs-column', './grid/bs-row', './grid/bs-container', './input-controls/bs-textarea', './input-controls/bs-form-group', './input-group/bs-input-group', './jumbotron/bs-jumbotron', './label/bs-label', './list-group/bs-list-group', './list-group/bs-list-group-item', './media-object/bs-media-object', './nav-bar/bs-nav-bar', './page-header/bs-page-header', './pagination/bs-pagination', './panel/bs-panel', './pill/bs-pill', './pill/bs-pill-item', './progress-bar/bs-progress-bar', './responsive-embed/bs-responsive-embed', './tab/bs-tab', './tab/bs-tab-item', './thumbnail/bs-thumbnail', './toolbar/bs-toolbar', './well/bs-well');
    }
    exports_1("configure", configure);
    return {
        setters:[
            function (BsAlert_1) {
                BsAlert = BsAlert_1;
            },
            function (BsBadge_1) {
                BsBadge = BsBadge_1;
            },
            function (BsBreadcrumb_1) {
                BsBreadcrumb = BsBreadcrumb_1;
            },
            function (BsBreadcrumbItem_1) {
                BsBreadcrumbItem = BsBreadcrumbItem_1;
            },
            function (BsButton_1) {
                BsButton = BsButton_1;
            },
            function (BsButtonGroup_1) {
                BsButtonGroup = BsButtonGroup_1;
            },
            function (BsDropDown_1) {
                BsDropDown = BsDropDown_1;
            },
            function (BsDropDownItem_1) {
                BsDropDownItem = BsDropDownItem_1;
            },
            function (BsFormGroup_1) {
                BsFormGroup = BsFormGroup_1;
            },
            function (BsGlyph_1) {
                BsGlyph = BsGlyph_1;
            },
            function (BsTextArea_1) {
                BsTextArea = BsTextArea_1;
            },
            function (BsInputGroup_1) {
                BsInputGroup = BsInputGroup_1;
            },
            function (BsJumbotron_1) {
                BsJumbotron = BsJumbotron_1;
            },
            function (BsLabel_1) {
                BsLabel = BsLabel_1;
            },
            function (BsListGroup_1) {
                BsListGroup = BsListGroup_1;
            },
            function (BsListGroupItem_1) {
                BsListGroupItem = BsListGroupItem_1;
            },
            function (BsMediaObject_1) {
                BsMediaObject = BsMediaObject_1;
            },
            function (BsNavBar_1) {
                BsNavBar = BsNavBar_1;
            },
            function (BsPageHeader_1) {
                BsPageHeader = BsPageHeader_1;
            },
            function (BsPagination_1) {
                BsPagination = BsPagination_1;
            },
            function (BsPanel_1) {
                BsPanel = BsPanel_1;
            },
            function (BsPill_1) {
                BsPill = BsPill_1;
            },
            function (BsPillItem_1) {
                BsPillItem = BsPillItem_1;
            },
            function (BsProgressBar_1) {
                BsProgressBar = BsProgressBar_1;
            },
            function (BsResponsiveEmbed_1) {
                BsResponsiveEmbed = BsResponsiveEmbed_1;
            },
            function (BsRow_1) {
                BsRow = BsRow_1;
            },
            function (BsColumn_1) {
                BsColumn = BsColumn_1;
            },
            function (BsContainer_1) {
                BsContainer = BsContainer_1;
            },
            function (BsTab_1) {
                BsTab = BsTab_1;
            },
            function (BsTabItem_1) {
                BsTabItem = BsTabItem_1;
            },
            function (BsThumbnail_1) {
                BsThumbnail = BsThumbnail_1;
            },
            function (BsToolbar_1) {
                BsToolbar = BsToolbar_1;
            },
            function (BsWell_1) {
                BsWell = BsWell_1;
            }],
        execute: function() {
            exports_1("BsAlert", BsAlert);
            exports_1("BsBadge", BsBadge);
            exports_1("BsBreadcrumb", BsBreadcrumb);
            exports_1("BsBreadcrumbItem", BsBreadcrumbItem);
            exports_1("BsButton", BsButton);
            exports_1("BsButtonGroup", BsButtonGroup);
            exports_1("BsDropDown", BsDropDown);
            exports_1("BsDropDownItem", BsDropDownItem);
            exports_1("BsGlyph", BsGlyph);
            exports_1("BsFormGroup", BsFormGroup);
            exports_1("BsRow", BsRow);
            exports_1("BsColumn", BsColumn);
            exports_1("BsContainer", BsContainer);
            exports_1("BsInputGroup", BsInputGroup);
            exports_1("BsJumbotron", BsJumbotron);
            exports_1("BsLabel", BsLabel);
            exports_1("BsListGroup", BsListGroup);
            exports_1("BsListGroupItem", BsListGroupItem);
            exports_1("BsMediaObject", BsMediaObject);
            exports_1("BsNavBar", BsNavBar);
            exports_1("BsPageHeader", BsPageHeader);
            exports_1("BsPagination", BsPagination);
            exports_1("BsPanel", BsPanel);
            exports_1("BsPill", BsPill);
            exports_1("BsPillItem", BsPillItem);
            exports_1("BsProgressBar", BsProgressBar);
            exports_1("BsResponsiveEmbed", BsResponsiveEmbed);
            exports_1("BsTab", BsTab);
            exports_1("BsTabItem", BsTabItem);
            exports_1("BsThumbnail", BsThumbnail);
            exports_1("BsTextArea", BsTextArea);
            exports_1("BsToolbar", BsToolbar);
            exports_1("BsWell", BsWell);
        }
    }
});

//# sourceMappingURL=index.js.map
